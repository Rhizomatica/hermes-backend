...
export class SessionsRepository {
  ...
  /**
   * Atomically mark the old session as replaced and insert a new session row.
   * Uses better-sqlite3's synchronous transaction callback (no await needed).
   */
  createAndReplace(
    oldSessionId: string,
    newSession: {
  ...
  ): { oldSession: SessionRow; newSession: SessionRow } {
    const newId = crypto.randomUUID();
    const newTokenHash = hashToken(newSession.refreshToken);
    const createdAt = new Date().toISOString();

    const oldSession = this.adapter.db
      .select()
      .from(userSessions)
